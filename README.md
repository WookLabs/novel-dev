...
### Micro-Turn Density 원고 게이트

`manuscript-micro-turn-density-not-evidenced`는 원고가 단서/기록/알림 같은 사실을 충분히 갖췄더라도 인접 문장 창이 독자 예측, 선택 압박, 위험, 관계 상태, 전술, 비용, 다음 질문을 자주 바꾸는지 봅니다. 3문장 안에서 가설 수정, 선택 축소, 위험 상승, 관계 변화, 다음 행동 변경, 대가, 다음 질문 중 하나로 이어지지 않는 창이 길게 반복되면 실패합니다. 사건 목록이 아니라 매 몇 문장마다 독자의 판단과 인물의 가능한 행동이 달라져야 합니다.
...
`uniform-paragraph-beat-cadence`는 문장 길이와 종결을 조금 바꿔도 인접 문단들이 같은 문장 수와 비슷한 장면 기능 순서로 반복되어 문단 박자가 AI처럼 고정될 때 별도 issue로 기록합니다. `max_uniform_paragraph_beat_run` 또는 CLI의 `--max-uniform-paragraph-beat-run`으로 허용치를 조정할 수 있고, style fingerprint에는 `longestUniformParagraphBeatRun`이 포함됩니다.
...
조사 절차 체크리스트 반복도 전체 문체 게이트에서 독립 profile로 조정합니다. `procedural-checklist-cadence`는 `max_procedural_checklist_density_per_1000`/`max_procedural_checklist_run` 또는 CLI의 `--max-procedural-checklists`/`--max-procedural-checklist-run`으로 허용치를 조정하며, 확인·검토·대조·정리 문장과 목적어-행동 로그가 가설 변화나 장면 결과 없이 반복되는 비율을 `proceduralChecklistDensityPer1000`, `longestProceduralChecklistRun`으로 남깁니다.
