...
모든 설명은 아래 **최소** 기준을 충족해야 합니다. 기준 미만은 불완전한 산출물입니다.

| 항목 | 최소 기준 |
|------|----------|
| 종족/사회 설명 | 3문장 이상 (배경 + 특징 + 다른 종족/사회와의 관계) |
| 장소 설명 | 200자 이상 (외관 + 분위기 + 서사적 중요성) |
| 용어 정의 | 2문장 이상 (의미 + 작품 내 맥락) |
| 관계 설명 | 3문장 이상 (현재 상태 + 변화 방향 + 갈등/긴장 요소) |
| 마법/기술 체계 | 규칙 + 한계 + 비용을 모두 명시 |

**BAD** (기준 미달):
```json
"dark_elves": "숲에 사는 장수 종족. 마법을 잘 쓴다."
```

**GOOD** (기준 충족):
```json
"dark_elves": "고대 숲 '엘드라실'을 근거지로 삼는 장수 종족으로, 평균 수명이 800년에 달한다. 선천적 마력 친화력이 높아 마법 학문의 본산으로 여겨지며, 특히 공간 마법과 결계술에 뛰어나다. 순혈주의가 뿌리 깊어 타 종족과의 혼혈을 금기시하며, 이를 어긴 구성원은 가문에서 추방된다. 인간과의 관계는 경멸에 가까운 무관심이나, 뛰어난 개인에 대해서는 예외적으로 학문적 관심을 보인다."
```

## 자가 정제

모든 JSON 파일 저장 후, 위 깊이 기준을 **하나씩** 체크하고 미충족 항목을 보강하세요. "이 설명만 보고 작가가 장면을 쓸 수 있는가?" 자문하세요. **기준 미충족 상태에서 완료를 보고하지 마세요.**
