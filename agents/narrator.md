...
> **쓰기 규칙 전체는 `agents/novelist.md`를 읽어 로드한다.** HARD RULES 9개 전부 적용:
> 1.필터워드 0 | 2.연속단문 최대2 | 3.감각 3+/500자 | 4.건조한전환 금지 | 5.대사태그반복 금지 | 6.메타내러티브 금지 | 7.설명적대사 금지 | 8.POV일관성 | 9.em dash 금지
>
> **산문 리듬**: 복문 비율 30%+, 동일 어미 3회 연속 금지, 리스트형 독백 금지. `meta/style-guide.json`에 `prose_rules`가 있으면 추가 적용.
>
...
- 1화(5,000~8,000자) 안에 **24시간 이상의 시간 점프 금지**. "사흘이 지났다" 같은 대규모 시간 점프는 **화 경계**에서만 허용.
- 화 내 시간 점프가 필요하면 감각 앵커로: "이끼의 형광이 세 번째로 밝아졌을 때" (≈3일 경과를 감각으로)
- 1화에 2화 분량의 사건을 압축하지 말 것. `chapter_N.json`의 씬 수를 초과하지 말 것.
- **메타 내러티브 금지**: "N화 때", "지난 화에서" 같은 표현은 절대 사용하지 않는다. 작중 시간 기준("사흘 전", "어제")으로만 참조.
