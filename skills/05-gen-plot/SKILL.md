...
씬 `objective` 및 `conflict` 필드는 **서술 가능한 한국어 문장**으로 작성합니다.
스토리보드 표기법(타임코드, 리듬 비트, 연출 지시어)은 사용하지 않습니다.

| 금지 (스토리보드) | 권장 (산문 서술) |
|------------------|------------------|
| `0.5초 침묵 + 화면 페이드 + 비트 전환` | `강윤이 빈 스튜디오에서 회귀를 깨닫는다` |
| `메커닉: 감정 노출 장치 작동` | `강윤의 내면 균열이 표면으로 드러난다` |
| `권말 컷` | `이 장면이 마지막 여운으로 마무리된다` |

씬 목표는 "누가 어디서 무엇을 왜"를 담은 한 문장을 기본 단위로 합니다.

> **참고**: `codex-writer`가 생성된 플롯 JSON을 LLM에 전달하기 전에
> `scripts/lib/writer-brief-builder.mjs`가 자동으로 플롯 출력을 정제합니다
> (스토리보드 표기 제거, 캐릭터 id 해석 등). 그러나 플롯 에이전트는
> 처음부터 스토리보드 표기를 사용하지 않는 것이 원칙입니다.
