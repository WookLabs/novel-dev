...
- scene active opposition / 능동 반대세력은 scene conflict/beat의 장면 압박 뒤에 적대적 의지(범인, 가해자, 적대 시스템, 내부자, 조작된 앱 등)를 두는 규칙입니다. 적대적 의지의 의도적 방해/조작/협박/표적화가 보이지 않고 날씨/정전/잠긴 문 같은 비의지적 장애물뿐이면 `scene-active-opposition-not-staged`, `chapters/chapter_XXX.md` 원고 본문 manuscript에서 빠지면 `manuscript-active-opposition-not-evidenced`를 critical로 표시합니다.
