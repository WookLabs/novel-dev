...
- micro-turn density는 인접 문장 창마다 독자 예측, 선택 압박, 위험, 관계 상태, 전술, 비용, 다음 질문 중 하나가 바뀌는지 확인하는 규칙입니다. `chapters/chapter_XXX.md` 원고 본문 manuscript가 단서/기록/알림을 나열하지만 3문장 안에서 가설 수정, 선택 축소, 위험 상승, 관계 변화, 다음 행동 변경, 대가, 다음 질문으로 이어지지 않으면 `manuscript-micro-turn-density-not-evidenced`를 critical로 표시합니다.
...
- 리듬 맵 검증: `chapters/chapter_XXX.md` 원고 본문 manuscript가 장면 압박에 맞춘 호흡 없이 같은 길이와 같은 종결, 같은 문장 수와 같은 문단 beat 구조의 AI식 규칙적 단문 반복이나 문단 반복으로 이어지면 문체/몰입 실패로 봅니다. 문장 길이 변주와 문단 기능 순서 변주가 압박-호흡-재점화를 만들어야 하며, 압박 구간은 행동/결과, 호흡 구간은 감각/물증/POV 앵커, 재점화 구간은 새 질문/위협/선택 비용을 보여야 합니다.
...
- `prose_taste_profile.max_uniform_paragraph_beat_run` 기준을 넘는 같은 문장 수/문단 기능 순서 반복은 `uniform-paragraph-beat-cadence` 문체 실패로 표시합니다. 수정본은 어미나 문장 길이만 바꾸지 말고 짧은 선택/충격 문단, 원인-행동-결과 문단, 물증-압박-재점화 문단처럼 문단 설계가 실제로 달라졌는지 확인합니다.
...
- `prose_taste_profile.max_procedural_checklist_density_per_1000` 또는 `max_procedural_checklist_run`을 넘는 확인/검토/대조/정리/분류 조사 절차 반복과 봉투/기록/번호를 옮겨 적었다/확인했다/표시했다식 목적어-행동 로그는 독자가 추론의 원인과 결과 대신 업무 목록을 읽게 만드는 문제이므로, `procedural-checklist-cadence`를 문체 실패로 표시하고 일부 확인 동작이 단서 불일치, 가설 변화, 용의자 재정렬, 상대 거짓말, 위험/비용, 다음 행동으로 바뀌었는지 확인합니다.
