...
function parseArgs(argv: string[]): CliArgs {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    } else if (arg === '--max-uniform-paragraph-beat-run' && argv[i + 1]) {
      profile.maxUniformParagraphBeatRun = parseIntegerInRange(
        argv[++i],
        '--max-uniform-paragraph-beat-run',
        1,
        20
      );
    ...
    } else if (arg === '--min-immersive-rhythm-anchors' && argv[i + 1]) {
      profile.minImmersiveRhythmAnchorDensityPer1000 = parseScore(
        argv[++i],
        '--min-immersive-rhythm-anchors'
      );
    } else if (arg === '--max-immersive-rhythm-flatline-run' && argv[i + 1]) {
      profile.maxImmersiveRhythmFlatlineRun = parseIntegerInRange(
        argv[++i],
        '--max-immersive-rhythm-flatline-run',
        1,
        20
      );
