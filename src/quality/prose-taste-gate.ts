...
const OBJECT_ACTION_LOG_PATTERN =
  /^(?:["“”'‘’「」『』\s]*)(?=[^.!?\n]{12,160}[.!?]?$)(?=[^.!?\n]*(?:봉투|숫자|사진|얼룩|통화\s*기록|빈칸|노트|녹음기|시간|신고\s*기록|카메라|번호|지도|열쇠|파일|기록|로그|주소|좌표|CCTV|카드|증거|단서|메시지|휴대폰|화면|문자|서류|영수증|신고서|출입\s*기록|사망\s*시각)[^.!?\n]{0,36}(?:을|를|와|과))(?=[^.!?\n]*(?:옮겨\s*적었|베껴\s*놓았|표시(?:해\s*두었|했)|맞춰\s*보았|확인했|검토했|대조했|검색했|조회했|분석했|정리했|분류했|기록했|추가했|살폈|열었|읽었|꺼냈|집었|넣고|넣었|챙겼|접었|펼쳤|스캔했|추적했|비교했|되감았|옮겼|놓았|두었|붙였|적었|썼|메모했|눌렀|걸어갔))[^.!?\n]+[.!?]?$/u;

const PROCEDURAL_CHECKLIST_TURN_PATTERN =
  /(?:하지만|그러나|그러자|그제야|대신|때문|탓에|바람에|비어\s*있|누락|빠져\s*있|불일치|맞지\s*않|틀렸|거짓|거짓말|무너졌|깨졌|바뀌었|달라졌|닫혔|열렸|사라졌|나타났|드러났|밝혀졌|발견|용의자|알리바이|범인|배후|함정|협박|위험|대가|신고(?:했|했다|하자|하고|한다|하려|하러|해야|했다가|해도|하면|했다면|하겠|를\s*(?:했|했다|하고|하자)|로\s*(?:이어|향))|체포|도주|추적|전화를\s*걸|문자를\s*보냈|현장으로\s*향|순서를\s*바꾸|계획을\s*바꾸|이름을\s*적었|멈췄|멈추었|확인하자|대조하자|맞춰\s*보자)/u;
...
function addProceduralChecklistCadenceIssue(
  ...
  issues.push({
    ...
    severity: severe ? 'high' : 'medium',
    message:
      `확인/검토/대조/정리 또는 목적어-행동 로그형 조사 동작이 ${checklistCount}회 감지되고 ` +
    ...
    suggestion:
      '조사 동작과 목적어-행동 로그 일부를 가설 변화, 단서 불일치, 용의자 재정렬, 상대의 거짓말, 위험/비용, 다음 행동으로 바꾸세요.',
    penalty: Math.min(
      24,
      9 +
        overflow * 5 +
...
function isProceduralChecklistSentence(sentence: string): boolean {
  ...
  if (
    !PROCEDURAL_CHECKLIST_PATTERN.test(trimmed) &&
    !OBJECT_ACTION_LOG_PATTERN.test(trimmed)
  ) {
    return false;
  }
