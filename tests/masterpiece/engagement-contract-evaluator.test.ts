...
const alignedManuscript = [
  ...
  '신고를 미룬 대가로 알리바이 선택지가 닫히고 공식 신고 기록이 사라져 그는 더는 안전한 제보자로 돌아갈 수 없었다. 제한 시간 안에 피해자를 찾으려 하자 예고 앱의 개발자가 통제선 앞 조명을 꺼뜨리고 현장 철문을 안쪽에서 잠갔고, 막힌 문턱 앞에서 그는 엘리베이터 계획을 접고 비상계단으로 우회해 피해자의 휴대폰 로그를 단서로 다음 행동을 바꿨다. 현장 실패가 상황을 되돌릴 수 없게 바꿨다.',
...
describe('evaluateEngagementContract', () => {
  ...
  it('fails when structural reward beats do not create reader desire for the outcome', () => {
    const result = evaluateEngagementContract({
      ...
      manuscript: [
        ...
        '신고를 미룬 대가로 알리바이 선택지가 닫히고 공식 신고 기록이 사라져 그는 더는 안전한 제보자로 돌아갈 수 없었다. 제한 시간 안에 피해자의 휴대폰 로그를 확보하려 하자 예고 앱의 개발자가 통제선 앞 조명을 꺼뜨리고 현장 철문을 안쪽에서 잠갔고, 막힌 문턱 앞에서 그는 엘리베이터 계획을 접고 비상계단으로 우회해 피해자의 휴대폰 로그를 단서로 다음 행동을 바꿨다. 현장 실패가 상황을 되돌릴 수 없게 바꿨다.',
  ...
  it('fails when active opposition is only an anonymous actor without concrete opposing identity', () => {
    const result = evaluateEngagementContract({
      design,
      plot,
      chapter: alignedChapter,
      manuscript: [
        '살인을 예고하는 앱의 첫 알림이 울리자 휴대폰 화면이 파란빛으로 번쩍였다. 앱이 아직 일어나지 않은 살인을 예보한다는 문장을 확인한 그는 장난으로 넘길지 위험을 감수하고 현장으로 갈지 선택해야 했다.',
        '공포보다 패턴을 먼저 읽은 주인공은 경찰 신고보다 현장 도착 시간을 계산했고, 첫 피해자 김하린의 목숨을 잃지 않게 하려 앱 알림이 실제 사건과 맞아떨어지는 첫 규칙 증명을 확인하려 뛰쳐나갔다. 경찰 신고를 택하면 현장 도착이 늦고, 현장을 택하면 신고 기록과 알리바이를 포기해야 했다.',
        '제한 시간 안에 피해자를 찾으려 하자 누군가 통제선 앞 조명을 꺼뜨리고 현장 철문을 안쪽에서 잠갔고, 막힌 문턱 앞에서 그는 엘리베이터 계획을 접고 비상계단으로 우회해 피해자의 휴대폰 로그를 단서로 다음 행동을 바꿨다.',
        '현장 실패가 첫 피해자의 사망 시각을 확정하며 상황을 되돌릴 수 없게 바꿨고, 단서가 맞물리는 지적 쾌감과 위기 직전의 긴장감이 목덜미를 조이고 손바닥에 식은땀을 끌어올렸다. 매 회차 작은 규칙 증명이라는 보상 주기는 현장 실패의 결과로 분명해졌다.',
        '피해자의 휴대폰에서 새 알림이 뜨며 주인공 이름이 다음 수신자로 다시 깜박이고, 과거 미제 사건 번호가 현재 사건 기록과 연결된다. 예고 앱의 개발자와 과거 미제 사건의 연결, 주인공 가족의 실종이 예고 앱의 장기 미스터리로 수렴한다는 단서가 남았고, 그는 화면을 쥔 채 왜 첫 수신자가 자신인지, 앱이 아직 벌어지지 않은 살인을 어떻게 알고 있었는지 알 수 없는 채 멈췄다.',
      ].join('\n'),
    });
    ...
    expect(result.revisionDirectives).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          ...
          priority: 'critical',
          target: 'manuscript',
          expected: expect.stringContaining('named opposing actor'),
        }),
      ])
    );
  });
