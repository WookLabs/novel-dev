...
const alignedChapter = {
  ...
  scenes: [
    ...
    },
    {
      ...
      conflict:
        '주인공은 제한 시간 안에 피해자를 찾으려 하지만 예고 앱의 개발자가 통제선 조명을 꺼뜨리고 현장 기록을 숨긴 탓에 피해자 휴대폰만 남는다.',
...
const convenientResolutionManuscript = [
  ...
  '제한 시간 안에 피해자를 찾으려 하자 예고 앱의 개발자가 통제선 앞 조명을 꺼뜨리고 현장 철문을 안쪽에서 잠갔고, 막힌 문턱 앞에서 그는 고립됐다.',
...
const nonPersonalizedStakesManuscript = [
  '살인을 예고하는 앱의 첫 알림이 울린 순간, 주인공은 장난으로 넘길지, 경찰에 신고해 알리바이를 남길지, 위험을 감수하고 현장으로 갈지 선택해야 했다.',
  '공포보다 패턴을 먼저 읽은 그는 경찰 신고보다 현장 도착 시간을 계산했고, 사람을 구하려 사무실 복도 끝에서 엘리베이터 버튼을 눌렀다가 지하보도 입구까지 젖은 계단을 내려가며 앱 알림이 실제 사건과 맞아떨어지는 첫 규칙 증명을 확인하려 뛰쳐나갔다.',
  '신고를 미룬 대가로 알리바이 선택지가 닫히고 공식 신고 기록이 사라져 그는 더는 안전한 제보자로 돌아갈 수 없었다. 제한 시간 안에 피해자를 찾으려 하자 예고 앱의 개발자가 통제선 앞 조명을 꺼뜨리고 현장 철문을 안쪽에서 잠갔고, 막힌 문턱 앞에서 그는 엘리베이터 계획을 접고 비상계단으로 우회해 현장 기록을 단서로 다음 행동을 바꿨다. 현장 실패가 상황을 되돌릴 수 없게 바꿨다.',
